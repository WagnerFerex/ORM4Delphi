...
Type
  TSimpleRTTI<T : class, constructor> = class(TInterfacedObject, iSimpleRTTI<T>)
    private
      ...
      {$ENDIF}
      function __BindValueToProperty( aEntity : TObject; aProperty : TRttiProperty; aValue : TValue) : iSimpleRTTI<T>;

      function __GetRTTIPropertyValue(aEntity : TObject; aPropertyName : String) : Variant;
    ...
    public
      ...
      function DataSetToEntity (aDataSet : TDataSet; var aEntity : TObject) : iSimpleRTTI<T>;
      {$IFNDEF CONSOLE}
...
function TSimpleRTTI<T>.__BindValueToProperty( aEntity : TObject; aProperty : TRttiProperty; aValue : TValue) : iSimpleRTTI<T>;
begin
...
function TSimpleRTTI<T>.__GetRTTIPropertyValue(aEntity: TObject;
...
function TSimpleRTTI<T>.DataSetToEntity(aDataSet: TDataSet;
  var aEntity: TObject): iSimpleRTTI<T>;
var
  Field : TField;
  teste: string;
  ctxRtti   : TRttiContext;
      ...
      begin
          ...
          begin
            ...
            begin
              ...
              prpRtti.SetValue(Pointer(TObject(aList[Pred(aList.Count)])), Value);
            end;
          end;
      end;
    finally
  ...
  try
    ...
    begin
      ...
      begin
        ...
        if Attribute is Campo then
          NameCampo := Campo(Attribute).Name;
      end;
      if IsPK then
        aWhere := aWhere + NameCampo + ' = :' + NameCampo + ' AND ';
    end;
  finally
