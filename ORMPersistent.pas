...
Type
  TSimpleDAO<T: class, constructor> = class(TInterfacedObject, iSimpleDAO<T>)
    ...
    public
      ...
      function Open : iSimpleDAO<T>; overload;
      function ApplyUpdate : iSimpleDAO<T>;
      {$IFNDEF CONSOLE}
...
uses
  SysUtils, ORMAttributes, TypInfo, ORMRTTI, ORMSQL;

{ TGenericDAO }

{$IFNDEF CONSOLE}
function TSimpleDAO<T>.ApplyUpdate: iSimpleDAO<T>;
begin
  FQuery.ApplyUpdate;
end;
...
begin
      ...
      FQuery.Open;
      TSimpleRTTI<T>.New(nil).DataSetToEntity(FQuery.DataSet, TObject(Result));
end;
...
//  begin
  ...
  try
    ...
    if Assigned(FForm) then
     TSimpleRTTI<T>
      .New(nil)
      .DataSetToEntity(FDataSource.DataSet, TObject(aEntity))
      .BindClassToForm(FForm, aEntity);
    {$ENDIF}
...
function TSimpleDAO<T>.Open: iSimpleDAO<T>;
begin
  Result := Self.Find;
end;
